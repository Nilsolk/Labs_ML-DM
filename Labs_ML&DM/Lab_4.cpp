...
void enter_edges(vector<Edge>& edges) {
    ...
    cout << "¬ведите рЄбра в формате (начало конец вес):\n";
    for (int i = 0; i < numEdges; i++) {
        Edge e;
        cin >> e.from >> e.to >> e.weight;

        if (in_edges(edges, e)) {
            cout << "ќшибка: –ебро " << e.from << " -> " << e.to << " уже существует. ¬ведите снова\n";
            i--;
            continue;
        }
        edges.push_back(e);
...
void dijkstra(vector<Edge> edges, char start, char end) {
    ...
    if (startIndex == -1 || endIndex == -1) {
        cout << "ќдна из вершин не найдена в графе.\n";
        return;
    }
    ...
    if (dist[endIndex] == INT_MAX) {
        cout << "ѕуть от " << start << " до " << end << " не существует.\n";
        return;
    }
    ...
    path = start + path;
    cout << path << endl;
    cout << "—уммарна€ стоимость: " << dist[endIndex] << endl;
}
